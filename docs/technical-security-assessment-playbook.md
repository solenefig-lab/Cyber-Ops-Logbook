...
| Enum Web       | `ffuf` / `gobuster`                                   | Fichiers / répertoires cachés (admin, backup, dev…)       | Accès à des interfaces ou fichiers non protégés | Gestion du cycle de vie applicatif, suppression des artefacts dev |
